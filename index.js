...
function __asyncjs__get_string_async_js(buffer_ptr,max_len) { return Asyncify.handleAsync(async () => { await new Promise(resolve => { let term = document.getElementById("output"); let inputContainer = document.getElementById("input-container"); let inputField = document.getElementById("cmd-input"); inputContainer.style.display = "flex"; inputField.focus(); let handleEnter = function(e) { if (e.key === "Enter") { inputField.removeEventListener("keydown", handleEnter); let val = inputField.value.trim(); inputField.value = ""; inputContainer.style.display = "none"; term.innerHTML += '<span style="color: #66fcf1; font-weight: bold;">> ' + (val === "" ? " " : val) + "</span><br>"; term.scrollTop = term.scrollHeight; stringToUTF8(val, buffer_ptr, max_len); resolve(); } }; inputField.addEventListener("keydown", handleEnter); }); }); }
...
var wasmImports = {
  /** @export */
  __asyncjs__get_string_async_js,
  /** @export */
